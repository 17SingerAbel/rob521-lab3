...
ROT_DIST_MULT = .02  # multiplier to change effect of rotational distance in choosing correct control
...
class PathFollower():
    ...
    def follow_path(self):
        #print(self.map_origin) 21 49.25
        while not rospy.is_shutdown():
            ...
            for opt in range(local_paths_pixels.shape[1]):
                ...
                for timestep in range(local_paths_pixels.shape[0]):
                    ...
                    y, x = disk(point, int(np.ceil(self.collision_radius_pix)), shape=map_shape)
            ...
            if final_cost.size == 0:  # hardcoded recovery if all options have collision
                ...
                #if curr_dist > MIN_TRANS_DIST_TO_USE_ROT:
                #    for i in range(len(valid_opts)):
                #        dist = np.linalg.norm(local_paths[-1,valid_opts[i],:2].reshape(1,2)
                #else:
                #    for i in range(len(valid_opts)):
                #        final_cost[i] += np.abs(local_paths[-1,valid_opts[i],2]-self.cur_goal[2])*ROT_DIST_MULT
                for i in range(len(valid_opts)):
                    ...
                    if dist_from_goal < MIN_TRANS_DIST_TO_USE_ROT:  
                        final_cost[i] += abs_angle_diff *0.01
                        #final_cost[i] += rot_dist_from_goal *0.02
                #for timestep in range(local_paths_pixels.shape[0]):
                 #   point = local_paths_pixels[timestep, i, :]
                 #   rr, cc = disk(point, int(self.collision_radius_pix*2),shape=map_shape)
