...
PATH_NAME = 'shortest_path_myhal_RRT_star.npy'  # saved path from l2_planning.py, should be in the same directory as this file
        ...
        while not rospy.is_shutdown():
            ...
            for t in range(1, self.horizon_timesteps+1): # for each timestep:
                for i in range(0, self.num_opts): # for each control option:
                    angle = local_paths[t-1,i,2]
                    local_paths[t,i,0] = local_paths[t-1,i,0] + np.cos(angle) * self.all_opts_scaled[i,0]
                    local_paths[t,i,1] = local_paths[t-1,i,1] + np.sin(angle) * self.all_opts_scaled[i,0]
                    local_paths[t,i,2] = local_paths[t-1,i,2] + self.all_opts_scaled[i,1]
            ...
            map_shape = self.map_np.shape #(49,159)
            #print(map_shape)
            #break
            ...
            for opt in range(local_paths_pixels.shape[1]):
                ...
                for timestep in range(local_paths_pixels.shape[0]):
                    ...
                    #print(point)
                    if point[1] < -3 or point[1] > map_shape[0] - 1:
                        coll = 1
                        break
                    x, y = disk(point, int(np.ceil(self.collision_radius_pix)), shape=map_shape)
                    if np.any(self.map_np[x,y]):
                        coll = 1
                        break
                if not coll:
            ...
            if final_cost.size == 0:  # hardcoded recovery if all options have collision
                ...
                for i in range(len(valid_opts)):
                    dist = np.linalg.norm(local_paths[-1,valid_opts[i],:2].reshape(1,2) - self.cur_goal[:2].reshape(1,2))
                    final_cost[i] = dist
                    abs_angle_diff = np.abs(local_paths[-1,valid_opts[i],2]- self.cur_goal[2])
                    #rot_dist_from_goal = min(np.pi * 2 - abs_angle_diff, abs_angle_diff)
                    if dist < MIN_TRANS_DIST_TO_USE_ROT:
                        final_cost[i] += abs_angle_diff *0.005
                        #final_cost[i] += rot_dist_from_goal *0.005
                    if dist_from_goal < MIN_TRANS_DIST_TO_USE_ROT:  
                        final_cost[i] += abs_angle_diff *0.02
                    #for timestep in range(local_paths_pixels.shape[0]):
                    #    point = local_paths_pixels[timestep, i, :]
                    #    x, y = disk(point, int(np.ceil(self.collision_radius_pix*2)),shape=map_shape)
                    #    if np.any(self.map_np[x,y]):
                    #        final_cost[i] += 0.02
                    #        break
                #curr_dist = np.linalg.norm(self.pose_in_map_np[:2] - self.cur_goal[:2])
                #for i in range(len(valid_opts)):
                #    dist = np.linalg.norm(local_paths[-1,valid_opts[i],:2].reshape(1,2) - self.cur_goal[:2].reshape(1,2))
                #    final_cost[i] += dist
                #    if dist < MIN_TRANS_DIST_TO_USE_ROT:   
                #        final_cost[i] = (local_paths[-1,valid_opts[i],2]-self.cur_goal[2])*ROT_DIST_MULT
                #for timestep in range(local_paths_pixels.shape[0]):
                #    point = local_paths_pixels[timestep, i, :]
                #    y, x = disk(point, int(np.ceil(self.collision_radius_pix*2)),shape=map_shape)
                #    if np.any(self.map_np[x,y]):
                #        final_cost[i] += 0.05
                #        break
                #for timestep in range(local_paths_pixels.shape[0]):
                 #   point = local_paths_pixels[timestep, i, :]
                  #  if point[1] < self.collision_radius_pix or point[1] > map_shape[0] - self.collision_radius_pix:
                   #     final_cost[i] += 0.5
                    #    break
